...
public class CmdVisaSetLadderRaceGameArea implements CommandExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command command, @NotNull String s, @NotNull String[] args) {
        ...
        if (args.length > 0 && args[0].equalsIgnoreCase("save")){
            ...
            String path = "LadderRace.GameArea.";
            String path2 = "LadderRace.world.";
            Area.save(path,editorMode.get(player).getLoc1(),editorMode.get(player).getLoc2());

            editorMode.remove(player);

            Eventminigame.getInstance().getConfig().set(path2+"name",player.getWorld().getName());
            Eventminigame.getInstance().getConfig().set(path2+"uuid",player.getWorld().getUID().toString());
            Eventminigame.getInstance().saveConfig();
            Eventminigame.getInstance().reloadConfig();
            player.sendMessage(Eventminigame.ChatColor("Data berhasil di simpan!!"));
        }else{
